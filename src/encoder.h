...
class Stripe {
 ...
 public:
  ...
  size_t CommandLength() { return this->fragments_[0].CommandDataLength(); }
...
// ECController controls the encoding/decoding process of a raft log entry by
...
class Encoder {
 ...
 public:
  ...
  // Encode a target log entry with specified k,m parameters and put
  // the generated encoding fragments into output vector. The memory
  // space for each fragment is internally allocated
  // Return ture if encoding succeeds, otherwise returns false
  bool EncodeEntry(const LogEntry& entry, Stripe* stripe);

  // Decode a series of fragments to generate the original log entry
  // the input is the source fragments, return the resultant log entry
  // by setting the input pointer
  bool DecodeEntry(Stripe* stripe, LogEntry* entry);

 private:
  // k, m is the parameter used in RS encoding process.
  // - k_: The number of fragments sharded by the controller
  // - m_: The number of extra parity generated by encoding process
  int k_, m_;
