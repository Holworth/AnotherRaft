...
bool Encoder::EncodeEntry(const LogEntry& entry, Stripe* stripe) {
  ...
  for (int i = 0; i < k_ + m_; ++i) {
    ...
    frag_entry.SetCommandData(Slice(nullptr, entry.CommandDataLength()));
    stripe->AddFragments(frag_entry);
  }
  return true;
}
...
bool Encoder::DecodeEntry(Stripe* stripe, LogEntry* entry) {
  ...
  char* data = new char[const_cast<Stripe*>(stripe)->CommandLength()];

  // First copy the complete data part
  std::memcpy(data, stripe->NotEncodedSlice().data(), stripe->NotEncodedSlice().size());
